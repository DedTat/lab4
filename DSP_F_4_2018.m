...
Q=0.2;%СКО шума
% kp=2;%количество периодов модельного сигнала
...
%     tic
    ...
    for i=1:kt %генерация модельного сигнала с экспоненциальной модуляцией
         w(i)=exp(-20*((i-kt/2)/kt)^2);%оконная функция   
         y(i)=cos(2*pi*kp*(i-kt/2)/kt)*w(i);%модельный сигнал
         y(i)=y(i)+Q*noise(i);%модельный сигнал с шумом                                               
    end
...
%     title('bpf')
    ...
    kp1=kpbpf;
    seach_area=0.7/kp1;%область поиска  относит. kp_bpf   
    ...
    %Вычисление массивов эталонных сигналов              
            for i=1:kt                    
                x(i)=cos(2*pi*j*(i-kt/2)/kt)*w(i);%эталонный сигнал
            end    
...
%       toc
      %СПЛАЙН-ИНТЕРПОЛЯЦИЯ И ПЕРЕДИСКРЕТИЗАЦИЯ
            xx=1:k;
            xi=1:0.1:k;             
            yint=interp1(xx,kor1,xi,'spline');% сплайн-интерполяция коэф корреляции               
            r1=kor1;       
